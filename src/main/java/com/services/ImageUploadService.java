...
import com.exeptoins.ImageNotFoundException;
...
public class ImageUploadService {
    ...
    public ImageModel uploadImageToUser(MultipartFile file, Principal principal) throws IOException {
        ...
        //перед загрузкой профайл фотографии проверить, звгружуена ли она уже
        ImageModel userProfileImage = imageRepository.findByUserId(user.getId()).orElse(null);
        //если место не пустое, то есть фотка присутствует, то удаляем
        if (!ObjectUtils.isEmpty(userProfileImage)) {
            imageRepository.delete(userProfileImage);
        }
        //задаем новую фотографию
        ImageModel imageModel = new ImageModel();
        imageModel.setUserId(user.getId());
        //прежде чем ее загрузить, мы ее уменьшим в размере
        imageModel.setImageBytes(compressBytes(file.getBytes()));
        imageModel.setName(file.getOriginalFilename());
        return imageRepository.save(imageModel);
    }
    //загрузить фотографию для поста
    ...
    //метод, который позволяет вернуть один единственный пост для юзера
